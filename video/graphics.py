...
for x in range(2):
    for y in range(2):
        bush.blit(temp2, (MULTY_PIXEL_V//8 + MULTY_PIXEL_V//4*x*2, MULTY_PIXEL_V//8 + MULTY_PIXEL_V//4*y*2))
...
for x in np.arange(1, WIDTH):
    pygame.draw.lines(background, color_lines, False, [(x * MULTY_PIXEL_V, 0), (x * MULTY_PIXEL_V, HEIGHT * MULTY_PIXEL_V)], 1)
for y in np.arange(1, HEIGHT):
    pygame.draw.lines(background, color_lines, False, [(0, y * MULTY_PIXEL_V), (WIDTH * MULTY_PIXEL_V, y * MULTY_PIXEL_V)], 1)
...
def play_video(game, VIDEO):
    ...
    for tank in game.team1:
        ...
        DISPLAY.blit(tower, (tower_rect[0] + tank.X * MULTY_PIXEL_V, tower_rect[1] + tank.Y * MULTY_PIXEL_V )) # (tank.X * MULTY_PIXEL_V + tower.get_width()//2, tank.Y * MULTY_PIXEL_V + tower.get_height()//2))
    ...
    for i in game.bullets_in_act:
        pygame.draw.circle(DISPLAY, (255,0,0), (game.bullets[i].X*MULTY_PIXEL_V,  game.bullets[i].Y*MULTY_PIXEL_V), 3)
