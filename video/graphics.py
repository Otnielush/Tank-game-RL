...
flag_red = pygame.transform.scale(flag_pic, (MULTY_PIXEL, MULTY_PIXEL))
flag_red.fill((255,0,0,100), special_flags=pygame.BLEND_ADD)
flag_blue = pygame.transform.scale(flag_pic, (MULTY_PIXEL, MULTY_PIXEL))
flag_blue.fill((0,0,255,100), special_flags=pygame.BLEND_ADD)

temp = tank_obs3.subsurface((144, 48, 15, 15))
land = pygame.transform.scale(temp, (MULTY_PIXEL, MULTY_PIXEL))
land.fill((139,69,19, 100), special_flags=pygame.BLEND_ADD)
temp = tank_obs.subsurface((116, 15, 28, 29))
rock = pygame.transform.scale(temp, (MULTY_PIXEL, MULTY_PIXEL))
temp = tank_obs.subsurface((30, 159, 28, 29))
wall = pygame.transform.scale(temp, (MULTY_PIXEL, MULTY_PIXEL))
temp = tank_obs.subsurface((58, 15, 28, 29))
forest = pygame.transform.scale(temp, (MULTY_PIXEL, MULTY_PIXEL)).convert_alpha()
...
water = pygame.transform.scale(temp, (MULTY_PIXEL, MULTY_PIXEL))
temp = tank_obs.subsurface((87, 362, 28, 29))
desert = pygame.transform.scale(temp, (MULTY_PIXEL, MULTY_PIXEL))
desert.fill((240, 240, 0, 100), special_flags=pygame.BLEND_MAX)
temp = tank_obs2.subsurface((192, 347, 32, 30))
temp2 = pygame.transform.scale(temp, (MULTY_PIXEL//3, MULTY_PIXEL//3))
bush = pygame.Surface((MULTY_PIXEL, MULTY_PIXEL))
for x in range(2):
    for y in range(2):
        bush.blit(temp2, (MULTY_PIXEL//8 + MULTY_PIXEL//4*x*2, MULTY_PIXEL//8 + MULTY_PIXEL//4*y*2))
# black is now transparent
bush.set_colorkey((0, 0, 0))
temp = tank_obs.subsurface((87, 362, 28, 29))
desert = pygame.transform.scale(temp, (MULTY_PIXEL, MULTY_PIXEL))
temp = tank_obs3.subsurface((112, 48, 15, 15))
swamp = pygame.transform.scale(temp, (MULTY_PIXEL, MULTY_PIXEL))
...
img_tank_base = pygame.transform.scale(pygame.image.load(os.path.join('video', 'pics', 'tankBase.png')), (tank_width, tank_height))
img_tank_tower = pygame.transform.scale(pygame.image.load(os.path.join('video', 'pics', 'tankTurret.png')), (round(tower_width), round(tower_height)))
...
def video_build_map(game):
    ...
    for x in range(WIDTH):
        for y in range(HEIGHT):
            ...
            if idd == 0:
                background.blit(land, (x*MULTY_PIXEL, y*MULTY_PIXEL))
            elif idd == 1:
                background.blit(rock, (x*MULTY_PIXEL, y*MULTY_PIXEL))
            elif idd == 0.86:
                background.blit(wall, (x * MULTY_PIXEL, y * MULTY_PIXEL))
            elif idd == 0.43:
                background.blit(forest, (x * MULTY_PIXEL, y * MULTY_PIXEL))
            elif idd == 0.57:
                background.blit(water, (x * MULTY_PIXEL, y * MULTY_PIXEL))
            elif idd == 0.29:
                background.blit(desert, (x * MULTY_PIXEL, y * MULTY_PIXEL))
            elif idd == 0.14:
                background.blit(bush, (x * MULTY_PIXEL, y * MULTY_PIXEL))
            elif idd == 0.71:
                background.blit(swamp, (x * MULTY_PIXEL, y * MULTY_PIXEL))

            if idd_base_red == 1:
                background.blit(flag_red, (x*MULTY_PIXEL, y*MULTY_PIXEL))
            elif idd_base_blue == 1:
                background.blit(flag_blue, (x * MULTY_PIXEL, y * MULTY_PIXEL))
...
def play_video(game):
    ...
    for tank in game.team1:
        ...
        DISPLAY.blit(tower, (tower_rect[0] + tank.X * MULTY_PIXEL, tower_rect[1] + tank.Y * MULTY_PIXEL )) # (tank.X * MULTY_PIXEL + tower.get_width()//2, tank.Y * MULTY_PIXEL + tower.get_height()//2))
    ...
    for tank in game.team2:
        ...
        tower = pygame.transform.rotate(img_tank_tower, (tank.direction_tank + tank.direction_tower) * 360)
        tower_rect = tower.get_rect()
        DISPLAY.blit(tower, (tower_rect[0] + tank.X * MULTY_PIXEL + tank.crop_x, tower_rect[
            1] + tank.Y * MULTY_PIXEL + tank.crop_y))  # (tank.X * MULTY_PIXEL + tower.get_width()//2, tank.Y * MULTY_PIXEL + tower.get_height()//2))
